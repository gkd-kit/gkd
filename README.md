...
学生入门增强入口会默认引导普通无障碍服务，帮助新用户一键导入推荐订阅并自动识别学校常用 App；它只会为识别或选择的已安装 App 写入开启配置，其他订阅支持 App 会保持关闭。订阅规则仍来自对应订阅维护者，使用和分发修改版时请保留上游来源、许可证和源码说明。详见 [学生入门说明](docs/student-onboarding/index.html)。
