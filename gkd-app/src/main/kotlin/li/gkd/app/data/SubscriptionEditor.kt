...
class SubscriptionEditor(subscription: RawSubscription) {
    ...
    fun replaceAppGroup(
        ...
        require(newGroup.key == groupKey) { UiStrings.rule_key_mismatch }
        ...
        val updated = updateAppGroup(targetApp.id, groupKey) {
            if (this != expectedGroup) error(UiStrings.rule_edit_conflict)
        ...
        if (!updated) error(UiStrings.rule_missing)
    }
    ...
    private fun setCurrent(subscription: RawSubscription) {
        require(subscription.id == subscriptionId) {
            UiStrings.subscription_id_immutable(subscriptionId, subscription.id)
        }
    ...
    private fun requireGroupNamesAvailable(
        ...
        newGroups.forEach { group ->
            if (!usedNames.add(group.name)) {
                error(UiStrings.rule_name_duplicate(group.name))
            }
        }
    }
